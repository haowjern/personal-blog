...
const debug = debugWithName("component:Editable"), PLACEHOLDER_STYLE = {
  ...
  const handleKeyDown = react.useCallback(
    ...
    ([, path]) => {
      ...
      const result = rangeDecorationState.filter((item) => slate.Range.isCollapsed(item) ? slate.Path.equals(item.focus.path, path) && slate.Path.equals(item.anchor.path, path) : slate.Range.intersection(item, { anchor: { path, offset: 0 }, focus: { path, offset: 0 } }) || slate.Range.includes(item, path));
